...
transformed parameters {
  ...
  lp_tilde = linear_predictor_re(N, X_tilde, alpha, region, v_tilde);
  lp = linear_predictor_re(N, X, beta, region, v);
  
  excessHaz = hazGG(N, time .* exp(lp_tilde), eta, nu, theta, 0) .* exp(lp);
  cumExcessHaz = cumHazGG(N, time .* exp(lp_tilde), eta, nu, theta) .* exp(lp - lp_tilde);
}
...
model {
  ...
  // Random effects
  target += normal_lpdf(v_tilde | 0, sigma_v_tilde);
  target += normal_lpdf(v | 0, sigma_v);
  
  // Hyperpriors
  target += lognormal_lpdf(sigma_v_tilde | 1, 1); 
